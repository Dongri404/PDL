...
class MetricsTop():
    ...
    def __eval_sims_regression(self, y_pred, y_true):
        test_preds0 = y_pred.view(-1).cpu().detach().numpy()
        test_truth0 = y_true.view(-1).cpu().detach().numpy()
        test_preds = np.clip(test_preds0, a_min=-1., a_max=1.)
        test_truth = np.clip(test_truth0, a_min=-1., a_max=1.)
        ...
        mae = np.mean(np.absolute(test_preds - test_truth))   # Average L1 distance between preds and truths
        corr = np.corrcoef(test_preds0, test_truth0)[0][1]
        # if np.isnan(corr):
        #     pdb.set_trace()
